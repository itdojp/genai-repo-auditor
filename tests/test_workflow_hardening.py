...
class WorkflowHardeningTests(unittest.TestCase):
    ...
    def test_lint_workflow_uses_explicit_read_only_permissions(self) -> None:
        ...
        self.assertIn('          assert len(load_corpus(root)["cases"]) >= 20', lint)
        self.assertIn('          from genai_repo_auditor.cli import COMMANDS', lint)
        self.assertIn('          test "$count" = "37"', lint)
    ...
    def test_install_matrix_workflow_exercises_supported_package_installs(self) -> None:
        ...
        self.assertIn('$env:PYTHONHASHSEED = "0"', package_install)
        self.assertIn("print(chr(10).join(COMMANDS))", package_install)
        self.assertIn("assert len(COMMANDS) == 37, COMMANDS", package_install)
        ...
        self.assertIn('assert Path(gra.resource_path("lib", "platform_support.py")).is_file()', package_install)
        ...
        self.assertIn('assert len(load_corpus(gra.resource_root())["cases"]) >= 20', package_install)
        self.assertIn('if ($commands.Count -ne 37)', package_install)
        self.assertIn('GRA_HOLDOUT_SMOKE_ROOT', package_install)
        self.assertIn('["gra-efficacy-holdout", "--records-root", os.fspath(root)]', package_install)
        self.assertIn('Private holdout records validated', package_install)
        ...
        self.assertIn("gra-doctor did not classify native Windows", package_install)
        self.assertIn("unsupported-fail-closed", package_install)
        self.assertIn("gra-doctor did not report workflow support", package_install)
        ...
        self.assertIn("native Windows efficacy generation must fail closed with status 2", package_install)
        self.assertIn("WSL2/Linux/macOS", package_install)
        ...
        self.assertIn('$workflowRun = Join-Path $auditRuns "acme__api/installed-prepare"', package_install)
        self.assertIn('Join-Path $workflowRun "run-manifest.json"', package_install)
        self.assertIn("gra-run --run $workflowRun --profile recon-only", package_install)
        self.assertIn("gra-run --run $workflowRun --profile recon-only --execute --until recon", package_install)
        self.assertIn("gra-run --run $workflowRun --profile recon-only --resume", package_install)
        self.assertIn('$pausedCheckpoint.status -ne "paused"', package_install)
        self.assertIn('$finalCheckpoint.status -ne "succeeded"', package_install)
        self.assertIn("gra-scan --run $workflowRun --tool gitleaks --plan", package_install)
        self.assertIn("GRA_MOCK_CODEX_FAIL_ONCE_FILE", package_install)
        self.assertIn("expected bounded workflow stage failure status 9", package_install)
        self.assertIn('$failedCheckpoint.status -ne "blocked"', package_install)
        self.assertIn('$recoveredCheckpoint.status -ne "succeeded"', package_install)
