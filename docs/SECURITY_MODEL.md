...
### Optional efficacy worker

efficacy benchmark の worker は `--compare --worker --worker-dir` を全て明示した場合だけ
実行し、builtin Codex profile だけを受け入れます。public-safe synthetic fixture を
設定済み model channel に渡し、approval `never`、read-only sandbox、web search disabled、
sandbox network access `false` を固定します。ephemeral session を使用し、user configuration と
project/user rule は読み込みません。prompt/schema/event/response/stderr artifact は指定した local
worker directory に残し、comparison report には含めません。model/control-plane channel を使用する
ため worker mode は offline ではありません。read-only sandbox は全 host-readable file の
confidentiality boundary ではないため、必要なら専用 host/container を使用します。詳細は
[`EFFICACY_CLAIMS_AND_PUBLICATION.md`](EFFICACY_CLAIMS_AND_PUBLICATION.md) を参照してください。
operator は trusted `PATH` を用意し、解決された `codex` executable と model service を trusted
dependency として扱います。worker environment は縮小されますが、model authentication/proxy
variable は Codex の接続に必要な場合があります。
