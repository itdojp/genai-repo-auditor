...
ユーザーが書き込めるインストール先を決めます。この例では `$HOME/.local/opt` を使います。
...
今後の shell でも有効にする場合は、同じ行を `~/.profile`、`~/.bashrc`、`~/.zshrc` などに追加してください。
...
次のような audit run directory が表示されます。

```text
runs/OWNER__REPO/first-prepare/
```

生成された context を確認します。
...
`RUN_ID` は、`--run-id` を指定しない限り UTC timestamp と process identifier から生成されます。
