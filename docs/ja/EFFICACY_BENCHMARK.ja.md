...
## Private holdout との分離

この文書と `gra-efficacy-benchmark` は commit 済み public synthetic corpus 専用です。private holdout の fixture、case ID、実行 artifact を public corpus に追加してはいけません。private holdout は aggregate-only の [`PRIVATE_HOLDOUT_PROTOCOL.ja.md`](PRIVATE_HOLDOUT_PROTOCOL.ja.md) に従って別管理してください。
