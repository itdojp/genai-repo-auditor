...
| 4 | 2:15-4:15 | Synthetic benchmark summary | `bin/gra-efficacy-benchmark` and sanitized summary script | Deterministic public corpus, offline execution, bounded evidence | Show counts only, not raw fixture files or large JSON bodies | Any narration treats the result as production recall or model superiority |
| 5 | 4:15-5:45 | Fixed comparison summary | `bin/gra-efficacy-benchmark --compare` and sanitized summary script | One pinned stage difference, human review still required | Keep view on summary text only | Any narration claims a configuration is broadly superior |
