...
| Generated outputs | Text or JSON diagnostics on stdout, including `platform_support` and name-only `github_token_environment` checks. The run-directory probe creates and removes one temporary local file under `--runs-dir`. No audit run, cloned target repository, report, issue draft, or GitHub mutation is created. |
