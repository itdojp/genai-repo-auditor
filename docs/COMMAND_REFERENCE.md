...
| Generated outputs | Planning/new execution writes `<reports_dir>/workflow-plan.json` and `<reports_dir>/WORKFLOW_PLAN.md`; execution also writes `<reports_dir>/workflow-checkpoint.json`, `<reports_dir>/workflow-execution.json`, and `<reports_dir>/WORKFLOW_EXECUTION.md`. Resume validates and consumes the existing plan/checkpoint without rewriting the plan, then updates only checkpoint/execution status artifacts. The checkpoint retains resume-integrity stamps; the execution reports expose bounded stage status, duration, failures, scoped skips, blocked dependencies, absence reasons, and the resume stage. `gra-run` also appends a sanitized v2 completion event after planning or execution. None of these artifacts copy command output or raw payloads. |
...
| Security / disclosure cautions | Execution is opt-in. Only exact profile-approved local command tuples run; the orchestrator never adds network flags or invokes Issue/release/remediation publication. New execution rejects pre-existing declared outputs; use a fresh run or a supervised later `--from` range. Paused or blocked run state is checked before every stage. Resume rejects mismatched plans, changed command implementations, incomplete output stamp sets, and changed/missing successful-stage artifacts before any plan write; it does not repeat successful stages. Failed completion events list only artifacts changed by that invocation. Existing `gra-*` commands stay independently usable. |
