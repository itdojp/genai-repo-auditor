...
| Key options | Repeat `--allowed-prefix PREFIX` for paths that are intentionally in scope; `--out-md OUT` writes a Markdown final-check report; `--json` prints machine-readable output. |
