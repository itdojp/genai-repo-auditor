...
def append_coverage_normalization_log(run_dir: Path, changes: list[dict[str, Any]]) -> None:
    if not changes:
        return
    reports = reports_dir(run_dir)
    reports.mkdir(parents=True, exist_ok=True)
    timestamp = utc_now()
    jsonl_path = reports / "coverage-normalizations.jsonl"
    ...
    audit_log = reports / "AUDIT_LOG.md"
    lines = [
        "",
